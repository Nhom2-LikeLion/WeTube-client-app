...
import { useGetSubscribedChannelsQuery } from "@/app/api/subscriptionsApi";
            ...
            setBubbles((prev) => {
                ...
                for (let i = 0; i < next.length; i++) {
                    ...
                    for (let j = 0; j < next.length; j++) {
                        ...
                        const distToCenterJ = Math.sqrt(dxToCenterJ * dxToCenterJ + dyToCenterJ * dyToCenterJ);
                        const edgeRatioJ = 1 - Math.min(distToCenterJ / maxDist, 1);
                        const scaleJ = 0.5 + edgeRatioJ;

                        const minDist = (next[i].size * scaleI + next[j].size * scaleJ) / 2 + gap;
