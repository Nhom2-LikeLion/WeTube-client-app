...
export default function NewTrending() {
  ...
  useEffect(() => {
    const checkMobile = () => {
      ...
      let newDisplayCount;
      if (isExpanded) {
        newDisplayCount = isMobileDevice ? 6 : newTrendingList.length;
      } else {
  ...
  const visibleList = newTrendingList.slice(0, displayCount);
  ...
  return (
    <section className="px-4 sm:px-6 md:px-8 py-6">
      ...
      {((isMobile && newTrendingList.length > 2) ||
        (!isMobile && newTrendingList.length > 4)) && (
